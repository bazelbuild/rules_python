...
| <a id="whl_library-python_interpreter_targets"></a>python_interpreter_targets |  If you are using custom python interpreters built by another repository rule, use this attribute to specify its BUILD targets with platforms as keys . This allows pip_repository to invoke pip using the same interpreter as your host toolchain. If set, takes precedence over python_interpreter.   | <a href="https://bazel.build/rules/lib/dict">Dictionary: String -> String</a> | optional | <code>{}</code> |
