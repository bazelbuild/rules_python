...
""".format(
        ...
        coverage_tool = render.str(str(rctx.attr.coverage_tool)),
    )
    rctx.delete("python")
...
This now has a type of {type}`Label` instead of a simple {type}`str` so that users can
more easily provide their own coverage_tool.
:::
""",
        ),
