...
def _toolchain_aliases_impl(rctx):
    ...
    # In previous versions of this code we used PLATFORMS.each()
    # for the select statements below.  This gave use a list of all of
    # the platforms and not just the platforms supported by the
    # Python version.  For instance not all Python versions include
    # support for Power8 (ppc).
    # We now are using the reference to the actual Python repository
    # that is specific to the host platform.
...
""".format(
    ...
    if not is_windows:
        build_contents += """\
alias(name = "pip",  actual = "@{py_repository}_{host_platform}//:python_runtimes")
""".format(
            py_repository = rctx.attr.user_repository_name,
            host_platform = host_platform,
        )
