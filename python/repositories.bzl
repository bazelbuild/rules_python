...
def _uv_repo_impl(repository_ctx):
    suffix = ".zip" if repository_ctx.attr.platform == "windows" else ".tar.gz"
    ...
    url = "https://github.com/astral-sh/uv/releases/download/{version}/uv-{platform}{suffix}".format(
        version = repository_ctx.attr.uv_version,
        platform = repository_ctx.attr.platform,
        suffix = suffix,
    )
...
load("@rules_python//python:toolchain.bzl", "uv_toolchain")

uv_toolchain(
    name = "uv_toolchain",
    uv_tool = select({
        "@bazel_tools//src/conditions:host_windows": "uv.exe",
