...
def generate_gazelle_macro(
    ...
    Generates a gazelle.bzl file containing a macro (install_deps()) which instantiates
    ...
    return textwrap.dedent(
        (
            """\
        # Used to be passed to build the gazelle manifest.
        GAZELLE_PLUGIN_ARGS = struct(
            wheels = [{all_whl_requirements}],
            # FIXME @aignas 2023-01-03: the following is only used to generate a hash. Could we
            # do this here somehow and not need the requirements_lock at all?
            # TODO @aignas 2023-01-04: maybe we can do better than this?
            requirements = "//:{requirements_lock}",
            pip_repository_convention = "{public_repo}{repo_convention}",
            pip_target_convention = "{target_convention}",
        )
...
"""
            ...
            repo=repo,
            public_repo=repo_prefix[:-1],
            requirements_lock=Path(requirements_lock).name,
            repo_convention="" if bzlmod else "_%s",
            target_convention="%s_pkg" if bzlmod else "pkg",
        )
    )
...
def main(output: TextIO, gazelle_output: TextIO) -> None:
    """Args:
...
If set, it will take precedence over python_interpreter.",
    ...
    gazelle_output.write(
        textwrap.dedent(
            """\
        \"\"\"gazelle hooks for this python repository.
...
if __name__ == "__main__":
    with open("requirements.bzl", "w") as requirement_file:
        with open("gazelle.bzl", "w") as gazelle_file:
            main(requirement_file, gazelle_file)
