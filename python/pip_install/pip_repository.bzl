...
def _pip_repository_impl(rctx):
    ...
    # Write library overrides file
    library_overrides_file = rctx.path("library_overrides.json")
    rctx.file(library_overrides_file, rctx.attr.library_overrides)
...
""",
    ...
    "library_overrides": attr.string(
        doc = "Optional library overrides to apply to packages.",
    ),
...
def package_overrides(overrides):
    """Overrides apply to alias rules generated from a `pip_repository` rule.

    Args:
        overrides (dict, required): Dictionary mapping package names to either
            alternative default targets, or selection dictionaries. In this case,
            the dictionary will be merged with the default generated by `pip_repository`.

    Returns:
        str: A json encoded string of the provided overrides.
    """
    processed_overrides = {}

    for name, label_or_select in overrides.items():
        current = {}
        if type(label_or_select) == type(""):
            current["//conditions:default"] = label_or_select
        else:
            for select_key, select_name in label_or_select.items():
                current[select_key] = select_name

        processed_overrides[name] = current
    return json.encode(processed_overrides)
