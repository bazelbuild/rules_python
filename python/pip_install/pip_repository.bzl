...
def _pip_get_packages(rctx):
    ...
    return struct(
        ...
        parsed_requirements_txt = parsed_requirements_txt,
        packages = packages,
        bzl_packages = bzl_packages,
    )

def _pip_repository_impl(rctx):
    pkgs = _pip_get_packages(rctx)
    build_contents = _BUILD_FILE_CONTENTS
    macro_tmpl = ""
    all_whl_reqs_tmpl = ""
    incompat_aliases = rctx.attr.incompatible_generate_aliases

    # If we have @@ we have bzlmod
    bzlmod = str(Label("//:unused")).startswith("@@")

    # We have naming differces between bzlmod and non-bzlmod code.
    # We also create the package alias' differently as well.
    # This section of code handles building the different templates
    # That are included in repositories.bzl.
    if bzlmod and incompat_aliases:
        _pkg_aliases(rctx, rctx.attr.repo_name, pkgs.bzl_packages)
        macro_tmpl = "@@{name}//{{}}:{{}}".format(name = rctx.attr.name)
    elif bzlmod and not incompat_aliases:
        build_contents += _bzlmod_pkg_aliases(rctx.attr.repo_name, pkgs.bzl_packages)
        macro_tmpl = "@@{name}//:{{}}_{{}}".format(name = rctx.attr.name)
    elif incompat_aliases:
        _pkg_aliases(rctx, rctx.attr.name, pkgs.bzl_packages)
        macro_tmpl = "@{name}//{{}}".format(name = rctx.attr.name)
        all_whl_reqs_tmpl = "@{name}//{{}}:whl".format(name = rctx.attr.name)
    else:
        macro_tmpl = "@{name}_{{}}//:pkg".format(name = rctx.attr.name)
        all_whl_reqs_tmpl = "@{name}_{{}}//:whl".format(name = rctx.attr.name)

    # Here we are building the list of requirements and whl's that are
    # part of the requirements file for bzlmod.
    all_requirements = []
    all_whl_requirements = []
    if bzlmod:
        for p in pkgs.bzl_packages:
            all_requirements.append(macro_tmpl.format(p, p) if incompat_aliases else macro_tmpl.format(p, "pkg"))
            all_whl_requirements.append(macro_tmpl.format(p, "whl"))
    else:
        for p in pkgs.bzl_packages:
            all_requirements.append(macro_tmpl.format(p))
            all_whl_requirements.append(all_whl_reqs_tmpl.format(p))

    all_requirements = _format_repr_list(all_requirements)
    all_whl_requirements = _format_repr_list(all_whl_requirements)

    rctx.file("BUILD.bazel", build_contents)

    # In bzlmod we write a much smaller requirements.bzl file because
    # in the extension we are calling the whl_library repository rule
    # in the pip extension.  This is because of the differences between
    # bzlmod and using a WORKSPACE file.  In a WORKSPACE file we call
    # "install_deps" macro that is created in the pip_repository_requirements.bzl.tmpl
    # file.  But in a MODULES.bazel file we are not able to make a
    # call via a load statement, and must call whl_libary in the extension.

    # Select the different template.
    template = rctx.attr._pip_bzlmod_template if bzlmod else rctx.attr._pip_template
    if bzlmod:
        rctx.template("requirements.bzl", template, substitutions = {
            "%%ALL_REQUIREMENTS%%": all_requirements,
            "%%ALL_WHL_REQUIREMENTS%%": all_whl_requirements,
            "%%MACRO_TMPL%%": macro_tmpl,
            "%%NAME%%": rctx.attr.name,
            "%%REQUIREMENTS_LOCK%%": str(pkgs.requirements_txt),
        })
    else:
        import_line = 'load("@rules_python//python/pip_install:pip_repository.bzl", "whl_library")'

        # The following code including the building of the config dict
        # is building values that is used when install_dep is called.
        # The install_dep macro is created when the following template
        # is created. This includes the annotations, options and config.
        annotations = {}
        ...
        tokenized_options = []
        for opt in pkgs.parsed_requirements_txt.options:
            for p in opt.split(" "):
        ...
        config = {
            ...
            "repo": rctx.attr.name,
            "repo_prefix": "{name}_".format(name = rctx.attr.name),
...
""",
    ...
    "_pip_bzlmod_template": attr.label(
        default = ":pip_repository_requirements_bzlmod.bzl.tmpl",
    ),
    "_pip_template": attr.label(
